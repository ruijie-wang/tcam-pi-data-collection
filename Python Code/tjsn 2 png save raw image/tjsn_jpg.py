...
def process_tjsn_file(filepath, temp_mode): 
    ...
    if temp_mode == 'F':
        ...
        vmin = 50 # C = 10
        vmax = 90 # C = 32.2
        plt.imsave('eg.png', lum_img, cmap=ironblack_colormap)
    ...
    if temp_mode == 'C':
        ...
        # Save image 
        output_path = os.path.join(output_folder, os.path.basename(filepath).replace('.tjsn', '.png'))
        plt.savefig(output_path, dpi="figure", bbox_inches="tight", pad_inches = 0.1) # change the pad space; plt.savefig does not have quality parameter. 
        plt.close()
